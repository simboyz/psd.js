  ...
  parseChannel: (li, idx, count, rows, cols, isLayer) ->
    ...
    switch COMPRESSIONS[comp]
      when 'RLE'
        ...
        rlecountsData = @readf ">#{count * rows}H"
        for ch in [0...count]
          rlelenForChannel = arraySum rlecountsData, ch * rows, (ch + 1) * rows
          data = @read rlelenForChannel
    ...
    if chlen isnt null and @tell() isnt (chpos + 2 + chlen)
      Log.debug "currentpos: #{@tell()} should be #{chpos + 2 + chlen}"
      @seek chpos + 2 + chlen, false
